...
6. 실행:
   ...
   node server.js
   ```
   콘솔에 접속 주소 + QR이 나옵니다. `start.bat`으로 브라우저까지 자동 오픈.

## 자동 업데이트 & 배너

- `start.bat`으로 실행하면 시작할 때마다 `git pull`로 **자동 업데이트**됩니다 (git clone으로 설치한 경우).
- 이미 켜져 있는 서버에는 상단에 🔄 새 버전 칩이 떠서 알려줍니다 — 재시작하면 적용.
- 상단 배너는 개발자가 이 저장소의 `banner.json`을 수정하면 **모든 사용자에게 10분 내 반영**됩니다 (앱 업데이트 불필요).
